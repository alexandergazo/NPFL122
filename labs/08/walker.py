...
parser.add_argument("--policy_noise_clip", default=0.05, type=float, help='Policy smoothing clip.')
parser.add_argument("--policy_noise_sigma", default=0.05, type=float, help='Policy smoothing sigma.')
...
class Network:
    def __init__(self, env, args, rng):
...
class OrnsteinUhlenbeckNoise:
    ...
    def __init__(self, shape, mu, theta, sigma, rng):
    ...
    def sample(self):
        self.state += self.theta * (self.mu - self.state) + self.rng.normal(scale=self.sigma, size=self.state.shape)
        return self.state
...
def main(env, args):
    ...
    noise = OrnsteinUhlenbeckNoise(env.action_space.shape[0], 0, args.noise_theta, args.noise_sigma, rng)
    training = True
    while training:
        for _ in range(args.evaluate_each):
            ...
            noise.reset()
            while not done:
                action = network.predict_actions([state])[0] + rng.normal(scale=args.noise_sigma)
    ...
    import pickle, lzma
    with lzma.open('walker.model', 'wb') as model_file:
